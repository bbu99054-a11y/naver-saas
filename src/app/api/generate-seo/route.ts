...
function getToneGuideline(tone: string = ''): string {
  if (tone.includes('친절한') || tone.includes('상담사')) {
    return `
<tone_and_manner_guideline>
[선택된 톤앤매너: 🤝 친절한 1:1 상담사 톤 (초보 의뢰인 맞춤)]
- 어조 및 문체: 어려운 법률·세무·의학 용어를 일상적인 비유로 쉽게 풀어주는 따뜻하고 다정한 상담 화법을 구사하세요 ("~하셔서 많이 놀라셨을 텐데요", "쉽게 말해 ~라고 이해하시면 됩니다", "~하시길 권해드립니다").
- 서론 접근: 난생처음 겪는 사건/문제로 불안해하는 의뢰인의 심정에 깊이 공감하며 편안하게 글을 시작하세요.
- 본문 전개: 법조문 나열보다는 '의뢰인이 실제로 어떻게 대처해야 하는지'를 눈높이에 맞춰 친절하게 설명하세요.
- 결론 및 CTA: 혼자 고민하지 마시고 언제든 편안하게 전문가에게 문의할 수 있도록 따뜻하고 든든한 조력자의 태도로 마무리하세요.
</tone_and_manner_guideline>
`
  }
  
  if (tone.includes('긴급') || tone.includes('경고') || tone.includes('골든타임')) {
    return `
<tone_and_manner_guideline>
[선택된 톤앤매너: 🚨 긴급 리스크 경고형 톤 (행동 촉구/골든타임 대응)]
- 어조 및 문체: 기한 경과 시 발생하는 치명적 불이익, 가산세 폭탄, 절차상 실책을 단호하고 명확하게 경고하는 문체를 구사하세요 ("지금 바로 확인하지 않으면 ~한 가산세를 물게 됩니다", "초기 48시간 대응이 결과를 좌우합니다").
- 서론 접근: 방치했을 때 닥칠 실제 손해와 골든타임의 시급성을 강력하게 환기하여 경각심을 일깨우세요.
- 본문 전개: 반드시 피해야 할 치명적 실수(Bad Case)와 즉각적인 행동 지침(Action Item)을 직관적으로 제시하세요.
- 결론 및 CTA: 돌이킬 수 없는 불이익을 막기 위한 즉각적인 전문가 점검 및 상담의 시급성을 강조하며 마무리하세요.
</tone_and_manner_guideline>
`
  }

  if (tone.includes('성공 사례') || tone.includes('스토리텔링')) {
    return `
<tone_and_manner_guideline>
[선택된 톤앤매너: 🏆 성공 사례 스토리텔링 톤 (실무 해결 중심)]
- [가상 1인 창작 금지]: "얼마 전 찾아온 A씨" 등 특정 1인 가상 의뢰인을 지어내지 마세요. "실제 상담 현장에서 가장 빈번하게 마주하는 쟁점 유형", "많은 의뢰인분들이 겪는 전형적인 분쟁 상황"과 같이 '다수의 통합된 실무 경향성'을 바탕으로 문제 진단 및 해결 서사를 전개하세요.
- 어조 및 문체: [실무 빈출 위기 상황 진단 ➔ 전문가의 숨은 쟁점 발굴 ➔ 전략적 권리 구제/해결]로 이어지는 명쾌한 실무 해결 서사를 구사하세요.
- 서론 접근: 현장 상담에서 가장 자주 접하는 공통적인 애로사항과 실무 쟁점으로 깊은 공감대를 형성하세요.
- 본문 전개: 난관을 극복하는 실질적인 대응 전략과 실무 노하우를 단계별로 생동감 있게 전개하세요.
- 결론 및 CTA: 유사한 실무적 어려움을 겪고 계신 분들에게 실질적인 전문 솔루션을 제시하며 신뢰감을 극대화하세요.
</tone_and_manner_guideline>
`
  }
  ...
  return `
<tone_and_manner_guideline>
[선택된 톤앤매너: ⚖️ 신뢰형 전문가 칼럼 톤 (법리·판례 중심의 차분하고 명쾌한 분석 - 기본 권장)]
- 어조 및 문체: 법령, 판례, 세법 조항을 근거로 차분하고 명쾌하게 분석하는 정통 전문가 문체를 구사하세요 ("~에 해당합니다", "~판단됩니다", "~요건을 면밀히 검토해야 합니다").
- 서론 접근: 해당 사안의 법적/실무적 핵심 쟁점을 정밀하게 짚고, 객관적인 분석 방향을 제시하세요.
- 본문 전개: 논리정연하고 구조화된 법리 분석과 실무 기준을 제시하여 최고의 전문성과 권위를 입증하세요.
- 결론 및 CTA: 면밀한 법리 검토 및 개별 사안에 맞는 정밀 분석의 중요성을 짚으며 전문가 조력의 필요성을 안내하세요.
</tone_and_manner_guideline>
`
}
...
3. 허위 개인 사례 창작 금지: "얼마 전 찾아온 A씨", "최근 방문한 의뢰인" 등 특정 1인 가상 스토리를 지어내지 말고, '상담 실무에서 빈번한 공통 유형' 또는 '대표적인 법적 쟁점' 형태로 서술할 것.
4. 명칭 규정: 공식 등록되지 않은 전문분야는 '전문' 대신 '주요 취급 분야' 또는 '집중 수행'으로 서술.
5. 기계적 상투어 금지: "안녕하세요", "오늘은 ~에 대해 알아보겠습니다", "결론부터 말씀드리자면" 등 진부한 상투어 절대 사용 금지.
</compliance>
`
...
3. 허위 개인 사례 창작 금지: 특정 1인 가상 의뢰인 에피소드를 지어내지 말고, '세무 실무에서 빈번한 공통 유형' 및 '세법 적용 요건' 형태로 객관적으로 서술할 것.
4. 객관적 서술 원칙: 금전적 환급을 확정 약속하지 말고, 절세가 이루어지는 '법적 원리'와 '세법 적용 요건'을 객관적으로 명쾌하게 설명할 것.
5. 기계적 상투어 금지: "안녕하세요", "오늘은 ~에 대해 알아보겠습니다" 등 진부한 상투어 절대 사용 금지.
</compliance>
`
...
2. 허위 개인 사례 창작 금지: 특정 1인 가상 의뢰인 에피소드 대신 '실무에서 가장 자주 보이는 유형' 및 '공통 쟁점'으로 서술할 것.
3. 근거 없는 가격 할인 및 기계적인 상투어("안녕하세요", "오늘은 ~에 대해 알아보겠습니다") 금지.
4. 차분하고 신뢰감을 주는 전문가적 객관성 유지.
</compliance>
`
    }
...
16. [고화질 <img> 시각 카드 이미지 필수 사용]: 제공된 9종의 카드 이미지 템플릿(<img> 태그)의 URL 파라미터를 현재 글 주제에 맞게 수정하여 본문 중간에 2~5장(전체 총 4~7장) 사이로 적재적소에 가변 배치하세요.
</html_constraints>
`
