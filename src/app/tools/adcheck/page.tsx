      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-14">
        ...
        <div className="flex items-center justify-between gap-4 mb-8">
          <nav className="flex items-center space-x-2 text-xs text-slate-500 font-mono">
            ...
            <Link href="/tools" className="hover:text-slate-900 transition flex items-center gap-1">
              <Layers className="w-3.5 h-3.5 text-blue-600" />
              <span>Tools Hub</span>
            </Link>
            <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
            <span className="text-blue-600 font-semibold">AdCheck Scanner</span>
          </nav>

          <Link
            href="/tools"
            className="text-xs px-3.5 py-1.5 rounded-xl bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 shadow-sm transition flex items-center gap-1.5"
          >
        ...
        <div className="text-center space-y-4 mb-10 md:mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold shadow-xs">
            ...
            <span>2026 대한민국 전문직 법정 광고 규정 컴플라이언스 엔진</span>
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
            전문직 법정 광고 금칙어 <span className="text-blue-600">1초 스캐너</span>
          </h1>

          <p className="text-sm sm:text-base text-slate-600 max-w-2xl mx-auto leading-relaxed">
            포스팅 1건당 <strong>과태료 최대 5,000만원</strong> 및 <strong>자격정지 처분</strong> 방지 — <br className="hidden sm:inline" />
            변호사, 의사, 세무사, 노무사, 행정사 블로그 원고를 100% 무료로 즉시 진단하고 안전한 법률 대체어로 정제하세요.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-2.5 text-xs text-slate-600 pt-2 font-mono">
            <span className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center gap-1.5">
              <Scale className="w-3.5 h-3.5 text-blue-600" /> 변호사법 제23조
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center gap-1.5">
              <Stethoscope className="w-3.5 h-3.5 text-emerald-600" /> 의료법 제56조
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center gap-1.5">
              <Calculator className="w-3.5 h-3.5 text-amber-600" /> 세무사법 제12조
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center gap-1.5">
              <Briefcase className="w-3.5 h-3.5 text-purple-600" /> 공인노무사법
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center gap-1.5">
              <FileSpreadsheet className="w-3.5 h-3.5 text-teal-600" /> 행정사법 제14조
            </span>
          </div>
        </div>
        ...
        {/* 5. pSEO 롱테일 정보성 콘텐츠 (토스 스타일 클린 문서 레이아웃) */}
        <div className="mt-16 md:mt-24 space-y-12 border-t border-slate-200 pt-12">
          {/* 가이드 소개 */}
          <div className="space-y-3">
            <div className="inline-flex items-center gap-1.5 text-xs font-bold text-blue-600 uppercase tracking-wider font-mono">
            ...
            <p className="text-sm md:text-base text-slate-600 leading-relaxed">
              2026년 현재 보건복지부, 대한변호사협회, 국세청 및 공정거래위원회는 온라인 블로그, SNS, 네이버 플레이스에 대한
          ...
          {/* 직역별 위반 및 처벌 기준표 (클린 테이블) */}
          <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 overflow-hidden shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs md:text-sm text-slate-700">
                <thead className="bg-slate-100 text-slate-900 uppercase font-mono text-[11px] border-b border-slate-200">
                  <tr>
                    <th className="p-3.5">직역 구분</th>
                    <th className="p-3.5">근거 법령</th>
                    <th className="p-3.5">대표 적발 금칙어 사례</th>
                    <th className="p-3.5">법정 처벌 및 행정처분 수위</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  <tr className="hover:bg-slate-50 transition">
                    <td className="p-3.5 font-bold text-slate-900 flex items-center gap-1.5">
                      <Scale className="w-4 h-4 text-blue-600" /> 변호사
                    </td>
                    <td className="p-3.5 font-mono text-slate-500">변호사법 제23조</td>
                    <td className="p-3.5 text-rose-700 font-mono font-medium">100% 승소, 최고, 전관예우 암시, 수임료 파괴/할인</td>
                    <td className="p-3.5 text-slate-600">대한변협 징계처분(영구제명, 제명, 3년 이하 정직, 3천만원 이하 과태료)</td>
                  </tr>
                  <tr className="hover:bg-slate-50 transition">
                    <td className="p-3.5 font-bold text-slate-900 flex items-center gap-1.5">
                      <Stethoscope className="w-4 h-4 text-emerald-600" /> 의사 · 병원
                    </td>
                    <td className="p-3.5 font-mono text-slate-500">의료법 제56조</td>
                    <td className="p-3.5 text-rose-700 font-mono font-medium">치료효과 보장, 부작용 없음, 전후사진 무단게시, 환자체험담</td>
                    <td className="p-3.5 text-slate-600">1년 이하 징역 또는 1천만원 이하 벌금, 의료기관 업무정지 1~3개월</td>
                  </tr>
                  <tr className="hover:bg-slate-50 transition">
                    <td className="p-3.5 font-bold text-slate-900 flex items-center gap-1.5">
                      <Calculator className="w-4 h-4 text-amber-600" /> 세무사
                    </td>
                    <td className="p-3.5 font-mono text-slate-500">세무사법 제12조</td>
                    <td className="p-3.5 text-rose-700 font-mono font-medium">환급률 1위, 최저가 기장료, 평균 환급액 단정, 전직 국세청 강조</td>
                    <td className="p-3.5 text-slate-600">기획재정부 징계(등록취소, 2년 이하 정직, 1천만원 이하 과태료)</td>
                  </tr>
                  <tr className="hover:bg-slate-50 transition">
                    <td className="p-3.5 font-bold text-slate-900 flex items-center gap-1.5">
                      <Briefcase className="w-4 h-4 text-purple-600" /> 노무사
                    </td>
                    <td className="p-3.5 font-mono text-slate-500">공인노무사법 제20조</td>
                    <td className="p-3.5 text-rose-700 font-mono font-medium">100% 부당해고 구제, 무조건 합의, 착수금 0원 미끼 수임</td>
                    <td className="p-3.5 text-slate-600">고용노동부 징계(등록취소, 업무정지, 과태료), 표시광고법 과징금</td>
                  </tr>
                  <tr className="hover:bg-slate-50 transition">
                    <td className="p-3.5 font-bold text-slate-900 flex items-center gap-1.5">
                      <FileSpreadsheet className="w-4 h-4 text-teal-600" /> 행정사
                    </td>
                    <td className="p-3.5 font-mono text-slate-500">행정사법 제14조</td>
                    <td className="p-3.5 text-rose-700 font-mono font-medium">음주운전 100% 구제, 담당 공무원 인맥, 비자 발급 보장</td>
                    <td className="p-3.5 text-slate-600">행정안전부 자격정지 처분, 경찰 고발 및 손해배상 청구</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          {/* 자주 묻는 질문 FAQ (토스 스타일 클린 아코디언 카드) */}
