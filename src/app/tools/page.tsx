  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-blue-500 selection:text-white">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        {/* 상단 브레드크럼 */}
        <nav className="flex items-center space-x-2 text-xs text-slate-400 font-mono mb-8">
          <Link href="/" className="hover:text-slate-200 transition">
            Home
          </Link>
          <ChevronRight className="w-3.5 h-3.5 text-slate-600" />
          <span className="text-blue-400 font-medium">Tools Hub</span>
        </nav>
        ...
        <div className="text-center space-y-4 mb-14 md:mb-16">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-slate-400 font-mono pt-2">
            <span className="flex items-center gap-1.5 text-emerald-400">
              <CheckCircle2 className="w-4 h-4" /> 100% 무료 무제한
            </span>
            <span className="flex items-center gap-1.5 text-blue-400">
              <Lock className="w-4 h-4" /> 서버 저장 없는 로컬 처리
            </span>
            <span className="flex items-center gap-1.5 text-purple-400">
              <Zap className="w-4 h-4" /> 1초 즉시 연산
            </span>
          </div>
        </div>

        {/* CLS 방지용 표준 광고 컨테이너 (728x90) */}
        <div className="w-full min-h-[90px] flex flex-col items-center justify-center p-3 mb-10 rounded-2xl bg-slate-900/40 border border-dashed border-slate-800 text-center relative overflow-hidden">
          ...
          <div className="text-xs text-slate-500 flex items-center gap-2">
            ...
            <span>Google AdSense Verified Placement Slot</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {TOOLS.map((tool) => {
            ...
            return (
              <div
                key={tool.id}
                className={`group rounded-3xl p-6 md:p-8 bg-slate-900/70 border backdrop-blur-xl transition-all duration-300 relative flex flex-col justify-between overflow-hidden shadow-xl ${
                  tool.isLive
                    ? `${tool.borderGlow} hover:-translate-y-1 hover:shadow-2xl`
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-3 mb-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br ${tool.color} text-white shadow-lg`}
                    >
                  ...
                  <div className="text-xs font-semibold text-slate-400 mb-1 font-mono">{tool.target}</div>
                  <h2 className="text-xl font-bold text-white mb-2 group-hover:text-blue-300 transition">
                    {tool.title}
                  </h2>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {tool.features.map((feat, idx) => (
                      <span
                        key={idx}
                        className="text-[11px] px-2.5 py-1 rounded-lg bg-slate-800/70 border border-slate-700/60 text-slate-300 font-mono"
                      >
                        ✓ {feat}
                      </span>
                    ))}
                ...
                <div>
                  {tool.isLive ? (
                    <Link
                      href={tool.href}
                      className="w-full py-3 px-4 rounded-xl font-bold text-sm bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white shadow-lg shadow-blue-500/20 transition flex items-center justify-center gap-2 group-hover:scale-[1.01]"
                    >
                      <span>지금 바로 무료 진단하기</span>
                      <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition" />
                    </Link>
                  ) : (
                    <div className="w-full py-3 px-4 rounded-xl font-medium text-xs bg-slate-800/80 text-slate-400 text-center border border-slate-700/50">
        ...
        {/* B2B SaaS 메인 전환 배너 (TinyWow Model) */}
        <div className="rounded-3xl bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 border border-blue-500/30 p-8 md:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto flex-shrink-0">
              <Link
                href="/dashboard"
                className="px-6 py-3.5 rounded-xl font-bold text-sm bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-400 hover:to-indigo-500 text-white shadow-xl shadow-blue-500/25 transition text-center flex items-center justify-center gap-2"
              >
              ...
              <Link
                href="/pricing"
                className="px-5 py-3 rounded-xl text-xs font-semibold bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 transition text-center flex items-center justify-center gap-1.5"
              >
                <span>요금제 보기</span>
