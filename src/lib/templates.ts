...
/**
 * 1. 최상단 1:1 맞춤 썸네일 고해상도 이미지 (800x800 - Headless Serverless PNG)
 */
export function getTopThumbnailTemplate(): string {
  return `
[사진 1: 최상단 1:1 맞춤 썸네일 이미지 - HTML 본문 최상단 필수 삽입]
<img src="/api/card-image/render?type=MAIN_THUMBNAIL&category=2026+핵심+실무+가이드&title=송파구+병의원+세무기장+성실신고확인대상+핵심+검토&sub=놓치면+막대한+가산세가+부과되는+실무+기준+3가지&sig=(전문가+사무소명)" alt="2026 핵심 실무 가이드 대표 썸네일" style="display: block; width: 100%; max-width: 480px; margin: 20px auto 35px auto; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.08);" />
`.trim();
}

/**
 * 2. 3대 필수 요건 체크리스트 고해상도 이미지 (800x450 - Headless Serverless PNG)
 */
export function getChecklistCardTemplate(): string {
  return `
[사진 2: 3대 필수 요건 체크리스트 인포그래픽 이미지 - 본문 설명 직후 삽입]
<img src="/api/card-image/render?type=CHECKLIST&title=반드시+검토해야+할+3대+필수+체크리스트&points=01.+첫번째+핵심+요건:매출+5억원+이상+제출의무+발생|02.+두번째+핵심+요건:비급여+수입+선제적+검증+필수|03.+세번째+핵심+요건:성실신고+세액공제+적극+활용" alt="3대 필수 요건 체크리스트 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 25px auto; border-radius: 14px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);" />
`.trim();
}

/**
 * 3. Before vs After 비교 대비 고해상도 이미지 (800x450 - Headless Serverless PNG)
 */
export function getComparisonCardTemplate(): string {
  return `
[사진 3: Before vs After 비교 대비 인포그래픽 이미지 - 잘못된 대처 vs 올바른 해결 비교 시 삽입]
<img src="/api/card-image/render?type=COMPARISON&title=일반+기장+대리+vs+전문+성실신고+비교&extra1=단순+장부+작성+중심+세무조사+리스크&extra2=적격증빙+선제적+검증+및+세액공제+확보" alt="일반 기장 대리 vs 전문 성실신고 비교 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 25px auto; border-radius: 14px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);" />
`.trim();
}

/**
 * 4. 핵심 수치 & 감면율 대형 하이라이트 고해상도 이미지 (800x400 - Headless Serverless PNG)
 */
export function getHighlightStatCardTemplate(): string {
  return `
[사진 4: 핵심 수치 & 공제액 대형 하이라이트 인포그래픽 이미지 - 금액이나 세액/기간 강조 시 삽입]
<img src="/api/card-image/render?type=STAT_HIGHLIGHT&category=MEDICAL+TAX+STATISTIC&title=연+수입금액+5억+원+이상&sub=의료업+성실신고확인서+의무+제출+대상+적용&extra3=[법적+근거:+소득세법+제70조의2]" alt="핵심 수치 하이라이트 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 25px auto; border-radius: 14px; box-shadow: 0 8px 25px rgba(0,0,0,0.08);" />
`.trim();
}

/**
 * 5. 3단계 실무 행동 로드맵 고해상도 이미지 (800x450 - Headless Serverless PNG)
 */
export function getProcessFlowCardTemplate(): string {
  return `
[사진 5: 3단계 실무 행동 로드맵 인포그래픽 이미지 - 절차나 단계별 가이드라인 설명 시 삽입]
<img src="/api/card-image/render?type=PROCESS_FLOW&title=원스톱+사건+해결+3단계+실무+절차&points=1단계:+초기+사실관계+진단|2단계:+입증+서류+제출|3단계:+최종+절세+확정" alt="3단계 실무 절차 로드맵 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 25px auto; border-radius: 14px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);" />
`.trim();
}

/**
 * 6. 의뢰인 빈출 질문 & 팩트 해설 고해상도 이미지 (800x420 - Headless Serverless PNG)
 */
export function getQnACardTemplate(): string {
  return `
[사진 6: 의뢰인 빈출 질문 & 팩트 해설 인포그래픽 이미지 - 오해하기 쉬운 질문 해소 시 삽입]
<img src="/api/card-image/render?type=QNA&title=성실신고확인서+제출+기한은+언제까지인가요?&sub=성실신고확인대상자는+1개월+연장되어+6월+30일까지+신고·납부+가능합니다." alt="자주 묻는 질문과 전문가 팩트 해설 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 25px auto; border-radius: 14px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);" />
`.trim();
}

/**
 * 7. 골든타임 & 리스크 주의 경고 고해상도 이미지 (800x380 - Headless Serverless PNG)
 */
export function getWarningRiskCardTemplate(): string {
  return `
[사진 7: 골든타임 & 리스크 주의 경고 인포그래픽 이미지 - 기한 만료나 패널티 경고 시 삽입]
<img src="/api/card-image/render?type=WARNING_RISK&title=성실신고확인서+미제출+시+치명적+불이익+주의&sub=산출세액의+5%+가산세+즉시+부과+및+세무조사+우선+선정" alt="골든타임 및 패널티 리스크 주의 경고 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 25px auto; border-radius: 14px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);" />
`.trim();
}

/**
 * 8. 핵심 3줄 결론 요약 고해상도 이미지 (800x480 - Headless Serverless PNG)
 */
export function getKeyTakeawaysTemplate(): string {
  return `
[사진 8: 오늘의 핵심 3줄 결론 요약 인포그래픽 이미지 - 결론부 직전 필수 삽입]
<img src="/api/card-image/render?type=KEY_TAKEAWAYS&title=오늘+포스팅+핵심+3줄+요약&points=1️⃣+의료업+연+매출+5억+원+이상+시+성실신고확인서+제출+의무+발생|2️⃣+급여/비급여+정산+및+의료기기+상각+전문+세무사+필수|3️⃣+성실신고+확인비용+세액공제(60%,+120만+원+한도)+적극+활용" alt="오늘 포스팅 핵심 3줄 요약 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 30px auto; border-radius: 14px; box-shadow: 0 8px 22px rgba(0,0,0,0.08);" />
`.trim();
}

/**
 * 9. 하단 상담 유도 (CTA) & 찾아오시는 길 배너 고해상도 이미지 (800x480 - Headless Serverless PNG)
 */
export function getFooterBannerTemplate(): string {
  return `
[사진 9: 하단 상담 유도 및 찾아오시는 길 배너 인포그래픽 이미지 - 글 최하단 필수 삽입]
<img src="/api/card-image/render?type=CTA_FOOTER&category=EXPERT+CONSULTATION&title=송파구+병의원+세무기장+1:1+맞춤+진단&extra1=(사무소+대표번호)&extra2=(상세+주소)&extra3=(네이버+예약링크)" alt="전문 상담 유도 및 사무소 안내 배너 인포그래픽" style="display: block; width: 100%; max-width: 540px; margin: 35px auto 10px auto; border-radius: 14px; box-shadow: 0 10px 25px rgba(0,0,0,0.08);" />
`.trim();
}
